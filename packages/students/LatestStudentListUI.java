...
import cop3330.ui.LatestStudentListUI;
import cop3330.models.Student;

/**
 * The {@code LatestStudentListUI} class provides a graphical user interface window
 * for displaying, filtering, and managing a list of students.
 * 
 * <p>This UI allows users to:</p>
 * <ul>
 *   <li>View a list of students in a table</li>
 *   <li>Filter students by search query</li>
 *   <li>Reset the applied filter</li>
 *   <li>View all courses</li>
 *   <li>Display detailed information and registered courses of a selected student</li>
 *   <li>Add a new student</li>
 *   <li>Save all changes to persistent storage</li>
 * </ul>
 * 
 * <p>The UI uses a {@link SimpleController} to interact with student and course data.
 * Components are arranged with the student table in the center and control buttons in the bottom panel.</p>
 * 
 * <p>The window is centered on the screen and exits the application when closed.</p>
 */
public class LatestStudentListUI {
    ...
    /**
     * Constructs the UI window displaying all students.
     * Sets up buttons for filtering, resetting, viewing courses,
     * showing student details, adding students, and saving data.
     * The window is centered on the screen.
     * 
     * @param controller the controller instance for data access
     */
    public LatestStudentListUI(SimpleController controller) {
    ...
    private void openCourseListUI(ActionEvent e) {
        ...
        new LatestCourseListUI(this.controller, this.frame, studentID);
    }
    ...
    private void showCourses(ActionEvent e) {
        // open the StudentUI and pass the details of the selected row
        new LatestCourseListUI(this.controller, this.frame);
    }
    
    /**
     * Populates the JTable with student data from the provided model
     * and embeds it in a JScrollPane added to the main frame.
     * 
     * @param model the {@link StudentListModel} containing student data
     */
    ...
    /**
     * Prompts the user to enter information for a new student and
     * adds the student through the controller.
     * 
     * @param e the action event triggered by the user
     */
    private void addStudent(ActionEvent e) {
        ...
        if( id != null && !id.isEmpty() && name != null && !name.isEmpty() && status != null && !status.isEmpty() ) {
            if( status.equals("Graduate") )
                thesisTopic = JOptionPane.showInputDialog(this.frame, "Enter Thesis Topic:");
            else if( status.equals("Undergraduate") )
                yearLevel = JOptionPane.showInputDialog(this.frame, "Enter Year Level:");
            
            // does not handle invalid status
            this.controller.addNewStudent(id, name, status, thesisTopic, yearLevel);
    ...
    /**
     * Saves all changes through the controller and informs the user of the result.
     * 
     * @param e the action event triggered by the user
     */
